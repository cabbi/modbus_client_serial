...
class ModbusClientSerialRtu extends ModbusClientSerial {
  ...
  ModbusResponseCode _readResponseHeader(
    ...
    try {
      ...
      response.setRxData(rxData);
    } catch (ex) {
      ModbusAppLogger.severe(
          "Unexpected exception in reading RTU message Header", ex);
    }
  ...
  ModbusResponseCode _readResponsePdu(
    ...
    try {
      ...
      response.addRxData(rxData);
      return _checksumCheck(response);
    } catch (ex) {
      ModbusAppLogger.severe(
          "Unexpected exception in reading RTU message PDU", ex);
    }
    return ModbusResponseCode.requestRxFailed;
  }

  ModbusResponseCode _checksumCheck(_ModbusSerialResponse response) {
    var responseChecksum = response.checksum;
    var pduChecksum = computeCRC16(response.getRxData(includeChecksum: false));
    if (pduChecksum[0] != responseChecksum[0] ||
        pduChecksum[1] != responseChecksum[1]) {
      return ModbusResponseCode.requestRxWrongChecksum;
    }
    return ModbusResponseCode.requestSucceed;
  }

  static Uint8List computeCRC16(Iterable<int> bytes) {
    int wCRCWord = 0xFFFF;
    for (var byte in bytes) {
      int nTemp = (byte ^ wCRCWord) & 0xFF;
      wCRCWord = (wCRCWord >> 8) & 0xFFFF;
      wCRCWord = (wCRCWord ^ wCRCTable[nTemp ~/ 8][nTemp % 8]) & 0xFFFF;
    }
    return Uint8List.fromList([wCRCWord & 0xFF, (wCRCWord >> 8) & 0xFF]);
  }

  static const List<List<int>> wCRCTable = [
